...
**Pre-1.0.** Interfaces will move. Read [Honest limits](#honest-limits) before relying on it.

## Install

```bash
npm i -g @funes-tech/cli
```

The dist-tag is derived from the version: `0.3.0` carries no prerelease suffix, so it publishes
under `latest` and a bare install gets it. The `alpha` tag is frozen on the `0.1.0-alpha` line and
no longer tracks releases — ask for it only to pin one of those old builds.
