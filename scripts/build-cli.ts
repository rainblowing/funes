...
writeFileSync(join(OUT, "package.json"), JSON.stringify({
  ...
  engines: { node: ">=22.14" },
  // Windows is documented as unsupported and NO smoke job runs there, but without this npm installs
  // cheerfully on it and the user discovers the truth from a native-addon stack trace. Refusing the
  // install with npm's own EBADPLATFORM is the honest version of a caveat we already publish.
  // Removing this is how Windows support ships — after a windows-latest job exists, not before.
  os: ["darwin", "linux"],
