...
import { indexDir, vaultNewerThan } from "./reindex.ts";
...
const noteEmptyOrStale = (vault: string, st: { nodes: number; lastReindexAt: string | null }, hits: number, q?: string): void => {
  if (st.nodes === 0) {
  ...
  } else if (vaultNewerThan(vault, at)) {
...
const USAGE = `usage: funes <command> [options]        funes ${FUNES_VERSION}
  ...
  publish   --vault <path> [--home <index-dir>] [--force]
  ...
  remember  --vault <path> --title "T" [--body "..."|stdin] [--type t] [--tags a,b] [--source s]
              write a memory to out_memory/, recorded UNTRUSTED.
  supersede --vault <path> <oldId> --title "T" [--body "..."|stdin]
              write a successor and tombstone the old item.
  link      --vault <path> <fromId> <toId> [--type related_to]
  forget    --vault <path> <id> [--hard]
  ...
  Model cache: FUNES_MODEL_DIR (default ~/.twinkling/models), ~145MB on first run.`;
...
if (cmd === "--version" || cmd === "-v") {
  ...
  noteEmptyOrStale(vault, st, res.length, q);
  if (has("--json")) {
