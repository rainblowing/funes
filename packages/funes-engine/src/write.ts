...
/** Deterministic, collision-safe id under `memoryDir` (no timestamp -> stable for tests).
 *  `out_memory/<slug>-<hash6>`, hash = sha256 over title+body — ids persist in filenames
 *  forever, so the hash must be contractually stable (S0: was Bun.hash, which may change
 *  across Bun releases). */
export function memoryId(input: RememberInput, memoryDir: string): string {
  if (input.id) return input.id;
  const h = createHash("sha256").update(`${input.title}\n${input.body}`).digest("hex").slice(0, 6);
...
function render(data: Record<string, unknown>, body: string): string {
  ...
  for (const k of ["title", "type", "created", "updated", "trust", "tags", "sources",
    "superseded_by", "valid_until", "forgotten", "edges"]) {
...
export function writeMemoryItem(root: string, input: RememberInput, meta: MemoryMeta): MemoryItem {
  ...
  return { id, path: `${id}.md`, title: input.title, type: input.type ?? "memory", body, edges: input.edges };
}
