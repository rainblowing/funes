...
export class PublishedIndex {
  ...
  async with<T>(fn: (store: FunesIndexStore, generation: string | null) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      handle.leases--;
      if (handle.retired && handle.leases === 0) await handle.store.close().catch(() => {});
    }
  }
  ...
  async close(): Promise<void> {
    ...
    this.current = null;
    if (cur && cur.leases === 0) await cur.store.close().catch(() => {});
    else if (cur) cur.retired = true; // draining lease closes it
  }
}
