...
export const operations: Operation[] = createRegistry([
  ...
  op({
    name: "health",
    description: "Index health: node/edge counts, embedding signature, dirty-reindex flag.",
    args: z.object({}),
    readonly: true,
    served: "index", // store stats only — servable on a cross-star surface (exempt from the scope guard)
    async run(ctx) {
      return { vault: ctx.vault, ...(await ctx.store.stats()) };
    },
  }),
  op({
    name: "hotlist",
  ...
  op({
    ...
    args: z.object({
      ...
      volatile: z.boolean().describe("true if this is a STATE that later writes should replace (a rate, a plan); false/omitted for an EVENT, which is append-only and never goes stale").optional().transform((v) => v === true),
