...
export const operations: Operation[] = createRegistry([
  ...
  op({
    ...
    async run(ctx, args) {
      const { ids } = await ctx.funes.remember([{
        ...
        body: args.body,
        type: args.type,
        meta: { tags: strList(args.tags) }, // no trust here — FunesStore stamps untrusted
      }]);
  ...
  op({
    ...
    async run(ctx, args) {
      const { id } = await ctx.funes.supersede(args.oldId, { title: args.title, body: args.body, type: args.type });
