...
// is "unknown". Before the coalesce, that silently replaced the authenticated principal that made
// the write — so routine maintenance destroyed provenance on a schedule, with nothing to notice.
// Both paths are covered because they are different SQL: an unchanged item takes the metadata-sync
// update, a changed one takes the upsert.
test("an anonymous reindex does not erase a stamped write_actor", async () => {
  const dir = mkdtempSync(join(tmpdir(), "funes-actor-"));
  const db = join(dir, "index.db");

  const authed = await LibsqlStore.create(fake, db, { writeActor: "operator:ada" });
  await authed.remember([{ id: "a", title: "Retainer", body: "9500/mo", type: "note" }]);
  expect((await authed.indexedPage({ id: "a" }))!.writeActor).toBe("operator:ada");
  await authed.close();

  // hash-identical → the "unchanged" metadata-sync path
  const anon = await LibsqlStore.create(fake, db);
  await anon.remember([{ id: "a", title: "Retainer", body: "9500/mo", type: "note" }]);
  expect((await anon.indexedPage({ id: "a" }))!.writeActor).toBe("operator:ada");

  // body edited → the "changed" upsert path
  await anon.remember([{ id: "a", title: "Retainer", body: "11000/mo", type: "note" }]);
  expect((await anon.indexedPage({ id: "a" }))!.writeActor).toBe("operator:ada");
  await anon.close();

  // ...but a REAL actor still takes ownership: this is last-writer-wins among named principals,
  // not an immutable ledger. Saying which one it is matters more than the value being sticky.
  const other = await LibsqlStore.create(fake, db, { writeActor: "operator:bo" });
  await other.remember([{ id: "a", title: "Retainer", body: "12000/mo", type: "note" }]);
  expect((await other.indexedPage({ id: "a" }))!.writeActor).toBe("operator:bo");
  await other.close();
});
