...
function acquireInProcess(key: string): Promise<() => void> {
  ...
  return new Promise((resolve) => mux.waiters.push(() => resolve(releaser())));
}
...
export async function withScopedWriteLock<T>(resource: string, fn: () => Promise<T>, opts?: WriteLockOpts): Promise<T> {
  ...
  const releaseInProcess = await acquireInProcess(key);
  try {
    const lock = await acquireWriteLock(resource, opts);
    try {
